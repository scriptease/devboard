...
After install, any terminal:
    ...
    devboard logs web -f
    devboard project ls          # projects with on/off counts
    devboard project start web   # start, stop, restart or status one project
    devboard stop-all
...
- **CLI.** `ls [--json]`, `add`, `rm`, `pin`, `open`, `start`, `stop`, `restart`, `logs [-f]`, `project <start|stop|restart|status|ls>`, `start-all`, `stop-all`, `doctor`, `up`, `down`, `tray`. `project` drives one group instead of the whole board; `restart` is a stop followed by a start. `stop-all` pins unsaved running rows first, like the page. `doctor` reports bun, `lsof`/`ps`, who holds `:4242`, `~/.local/bin` on PATH, and the tray app.
